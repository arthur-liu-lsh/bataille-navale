...
public class BattleShipsAI implements Serializable {
    ...
    public Hit sendHit(int[] coords) {
        ...
        // already found strike & Direction?
        if (lastVertical != null) {
            ...
            // if already found a strike, without Direction
        ...
        if (hit != Hit.MISS) {
            if (lastStrike != null) {
                lastVertical = guessDirection(lastStrike, res);
            }
            lastStrike = res;
        }
    ...
    private boolean guessDirection(int[] c1, int[] c2) {
        return c1[0] == c2[0];
    }
